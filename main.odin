...
main :: proc() {
	...
	visual_map := make_default_visual()
	collision_map := make_default_collision()

	loaded := load_level(LEVEL_PATH, &visual_map, &collision_map)
	if loaded {
	...
	for !rl.WindowShouldClose() {
		...
		case .Play:
			...
			update_player_collisions(&player, collision_map, dt)
		...
		case .Edit:
			...
			editor_update(&editor, &visual_map, &collision_map, editor_camera_temp, dt)
		}
		...
		draw_tilesets(visual_map)
		// Collision overlay in edit collision mode
		if game_state == .Edit {
			editor_draw_debug_overlay(editor, visual_map, collision_map)
		}

		if editor.show_grid {
			draw_grids(true, false, false)
		}
		...
		if game_state == .Edit {
			editor_draw_palette(editor, visual_map, collision_map)
		} else {
			...
			rl.DrawText(fmt.ctprintf("Map %dx%d Tile %d Dual %d  Sets %d", MAP_WIDTH, MAP_HEIGHT, TILE_SIZE, DUAL_OFFSET, len(TILESET_DECLS)), 10, 48, 12, rl.GRAY)
		}
		...
		// Footer: show load status
		any_ready := false
		for id in 0..<len(tileset_runtime) {
			if tileset_runtime[id].ready {any_ready = true; break}
		}
		if !any_ready {
			rl.DrawText("DEBUG COLORS (no texture)", 10, rl.GetScreenHeight()-50, 14, {255, 180, 80, 255})
		} else {
			rl.DrawText(fmt.ctprintf("%d tileset(s)  D:debug", len(tileset_runtime)), 10, rl.GetScreenHeight()-50, 14, {120, 200, 255, 255})
		}
	}
}
