...
editor_update :: proc(s: ^EditorState, m: ^Map, camera: rl.Camera2D, dt: f32) {
	...
	if inside {
		if rl.IsMouseButtonDown(.LEFT) {
			if s.selected_set >= 0 {
				m[cell[1]][cell[0]] = TileCell {
					set_id = s.selected_set,
				}
			} else {
...
editor_draw_world_hover :: proc(s: EditorState) {
	...
	rl.DrawText(
		fmt.ctprintf("%d,%d", s.hover_cell[0], s.hover_cell[1]),
		i32(px + 2),
		i32(py + 2),
		12,
		rl.WHITE,
...
editor_draw_debug_overlay :: proc(s: EditorState, m: Map) {
	...
	rl.DrawText(
		fmt.ctprintf("DEBUG %s  0=outer 15=inner  D toggle", TILESET_DECLS[s.selected_set].name),
		-30,
		-36,
		16,
		rl.YELLOW,
...
editor_draw_palette :: proc(s: EditorState, m: Map) {
	...
	if oild_id >= 0 {
		...
		if rt.ready {
			...
			rl.DrawTexturePro(
				rt.texture,
				src_inner,
				{f32(x), f32(y), f32(swatch_size), f32(swatch_size)},
				{0, 0},
				0,
				rl.WHITE,
			)
			...
			rl.DrawTexturePro(
				rt.texture,
				src_outer,
				{f32(x + swatch_size + 8), f32(y), f32(swatch_size), f32(swatch_size)},
				{0, 0},
				0,
				rl.WHITE,
			)
	...
	rl.DrawText(
		"LMB:Inner  RMB:Outer  MMB:Pick  G:Grid T:Tileset D:Debug",
		instr_x,
		10,
		13,
		rl.LIGHTGRAY,
	...
	rl.DrawText(
		"S:Save L:Load C:Clear R:Reset TAB:Play  1:oild 0:Outer",
		instr_x,
		28,
		13,
		rl.LIGHTGRAY,
	...
	if s.show_tileset && s.selected_set >= 0 && tileset_is_ready_by_id(s.selected_set) {
		...
		rl.DrawText(
			fmt.ctprintf("%s (%s)", rt.decl.name, rt.decl.file),
			panel_x + 6,
			panel_y + 4,
			14,
			rl.WHITE,
		...
		props := fmt.ctprintf(
			"color solid:%v slower:%v %s",
			rt.decl.is_solid,
			rt.decl.is_slower,
			rt.decl.has_remap ? "[remapped]" : "",
		)
		...
		if rt.ready {
			...
			for phys in 0 ..< 16 {
				...
				row := phys / 4
				dst := rl.Rectangle {
					f32(grid_x + i32(col) * cell),
					f32(grid_y + i32(row) * cell),
					f32(cell),
					f32(cell),
				}
			...
			for logical in 0 ..< 16 {
				...
				row := logical / 4
				dst := rl.Rectangle {
					f32(logic_x + i32(col) * cell),
					f32(logic_y + i32(row) * cell),
					f32(cell),
					f32(cell),
				}
				...
				rl.DrawText(
					...
					i32(dst.y + 2),
					10,
					phys == logical ? rl.GRAY : rl.YELLOW,
				)
				if phys != logical {
					rl.DrawText(
						fmt.ctprintf("p%d", phys),
						i32(dst.x + 2),
						i32(dst.y + 12),
						10,
						{150, 150, 255, 255},
	...
	if s.is_hover_valid {
		...
		if !cell_is_empty(cell) {
			...
			if s.selected_set >= 0 {
				dual_idx = get_dual_index_for_set(
					m,
					s.hover_cell[0],
					s.hover_cell[1],
					s.selected_set,
		...
		hover_txt = fmt.ctprintf(
			"Hover %d,%d | %s | Selected: %s | Dual:%d | %s",
			s.hover_cell[0],
			s.hover_cell[1],
			hover_desc,
			sel_desc,
		...
		hover_txt = fmt.ctprintf(
			"Selected %s | %s | Mode: EDIT (TAB to Play)",
			sel_desc2,
			s.status_msg,
		)
	}
